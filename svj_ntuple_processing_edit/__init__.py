...
def calculate_mt_jet_sum(jet1_pt, jet1_eta, jet1_phi, jet1_e, jet2_pt, jet2_eta, jet2_phi, jet2_e, met, metphi):
    """
    Calculates the transverse mass MT for the sum of two jets and MET
    """
    # Calculate the transverse components of MET and jet momenta
    met_x = np.cos(metphi) * met
    met_y = np.sin(metphi) * met
    jet1_x = np.cos(jet1_phi) * jet1_pt
    jet1_y = np.sin(jet1_phi) * jet1_pt
    jet2_x = np.cos(jet2_phi) * jet2_pt
    jet2_y = np.sin(jet2_phi) * jet2_pt

    # Calculate the transverse energies of the jets
    jet1_pz = jet1_pt * np.sinh(jet1_eta)
    jet1_transverse_e = np.sqrt(jet1_e**2 - jet1_pz**2)

    jet2_pz = jet2_pt * np.sinh(jet2_eta)
    jet2_transverse_e = np.sqrt(jet2_e**2 - jet2_pz**2)

    # Calculate the total transverse energy and momentum
    total_transverse_e = jet1_transverse_e + jet2_transverse_e + met
    total_transverse_px = jet1_x + jet2_x + met_x
    total_transverse_py = jet1_y + jet2_y + met_y

    # Calculate the transverse mass
    mt = np.sqrt(total_transverse_e**2 - total_transverse_px**2 - total_transverse_py**2)
    return mt
...
def filter_zprime_in_cone(array):
    ...
    #eta_subsubl = a['JetsAK15.fCoordinates.fEta'][:,2].to_numpy()
    #phi_subsubl = a['JetsAK15.fCoordinates.fPhi'][:,2].to_numpy()
...
def bdt_feature_columns(array, load_mc=True, save_scale_weights=False):
    ...
    # Prepare features
    arr = array.array
    # place requirements on gen particles
    # at least 1 zprime particle
    arr = arr[ak.sum(arr['GenParticles_PdgId']==4900023, axis=-1)>=1]
    # at least 2 dark quarks
    arr = arr[ak.sum((np.abs(arr['GenParticles_PdgId'])==4900101) & (arr['GenParticles_Status']==71), axis=-1)>=2]
    a = {}
    ...
    a['terzo_pt'] = ak.pad_none(arr['JetsAK15.fCoordinates.fPt'], target=3)[:,2].to_numpy()
    a['terzo_eta'] = ak.pad_none(arr['JetsAK15.fCoordinates.fEta'], target=3)[:,2].to_numpy()
    a['terzo_phi'] = ak.pad_none(arr['JetsAK15.fCoordinates.fPhi'], target=3)[:,2].to_numpy()
    a['terzo_e'] = ak.pad_none(arr['JetsAK15.fCoordinates.fE'], target=3)[:,2].to_numpy()
    a['terzo_mass'] = calculate_mass(a['terzo_pt'], a['terzo_eta'], a['terzo_e'])
    a['terzo_mt'] = calculate_mt(
        a['terzo_pt'], a['terzo_eta'], a['terzo_phi'], a['terzo_e'],
        a['met'], a['metphi']
        )
    ...
    a['terzo_subl_mt'] = calculate_mt_jet_sum(
        a['pt'], a['eta'], a['phi'], a['e'],
        a['terzo_pt'], a['terzo_eta'], a['terzo_phi'], a['terzo_e'],
        a['met'], a['metphi']
        )
    ...
    # Truth tagging jets
    # at least 1 zprime particle
#    true = arr[ak.sum(arr['GenParticles_PdgId']==4900023, axis=-1)>=1]
    # at least 2 dark quarks
#    true = true[ak.sum((np.abs(true['GenParticles_PdgId'])==4900101) & (true['GenParticles_Status']==71), axis=-1)>=2]
    eta_zprime = arr['GenParticles.fCoordinates.fEta'][arr['GenParticles_PdgId']==4900023][:,0].to_numpy()
    phi_zprime = arr['GenParticles.fCoordinates.fPhi'][arr['GenParticles_PdgId']==4900023][:,0].to_numpy()
    select_darkquarks = (np.abs(arr['GenParticles_PdgId'])==4900101) & (arr['GenParticles_Status']==71)
    eta_darkquark1 = arr['GenParticles.fCoordinates.fEta'][select_darkquarks][:,0].to_numpy()
    phi_darkquark1 = arr['GenParticles.fCoordinates.fPhi'][select_darkquarks][:,0].to_numpy()
    eta_darkquark2 = arr['GenParticles.fCoordinates.fEta'][select_darkquarks][:,1].to_numpy()
    phi_darkquark2 = arr['GenParticles.fCoordinates.fPhi'][select_darkquarks][:,1].to_numpy()

    # With the dark quarks, need to match and create a mask and save as "tagged half truth"
    a['subl_half_truth_tag'] = (
        ((calc_dr(a['eta'], a['phi'], eta_zprime, phi_zprime)<=1.5)
        | (calc_dr(a['eta'], a['phi'], eta_darkquark1, phi_darkquark1)<=1.5)
        | (calc_dr(a['eta'], a['phi'], eta_darkquark2, phi_darkquark2)<=1.5))
        )

    # With the dark quarks, need to match and create a mask and save as "tagged half truth"
    a['terzo_half_truth_tag'] = (
        ((calc_dr(a['terzo_eta'], a['terzo_phi'], eta_zprime, phi_zprime)<=1.5)
        | (calc_dr(a['terzo_eta'], a['terzo_phi'], eta_darkquark1, phi_darkquark1)<=1.5)
        | (calc_dr(a['terzo_eta'], a['terzo_phi'], eta_darkquark2, phi_darkquark2)<=1.5))
        )

    print(a['subl_half_truth_tag'])

    # With the dark quarks, need to match and create a mask and save as "tagged half truth"
    a['lead_half_truth_tag'] = (
        ((calc_dr(a['leading_eta'], a['leading_phi'], eta_zprime, phi_zprime)<=1.5)
        | (calc_dr(a['leading_eta'], a['leading_phi'], eta_darkquark1, phi_darkquark1)<=1.5)
        | (calc_dr(a['leading_eta'], a['leading_phi'], eta_darkquark2, phi_darkquark2)<=1.5))
        )

    # tag jets with the Z' & dark quarks fully inside
    a['subl_full_truth_tag'] = (
        (calc_dr(a['eta'], a['phi'], eta_zprime, phi_zprime)<=1.5)
        & (calc_dr(a['eta'], a['phi'], eta_darkquark1, phi_darkquark1)<=1.5)
        & (calc_dr(a['eta'], a['phi'], eta_darkquark2, phi_darkquark2)<=1.5)
        )
    ...
    # tag jets with the Z' & dark quarks fully inside
    a['lead_full_truth_tag'] = (
        (calc_dr(a['leading_eta'], a['leading_phi'], eta_zprime, phi_zprime)<=1.5)
        & (calc_dr(a['leading_eta'], a['leading_phi'], eta_darkquark1, phi_darkquark1)<=1.5)
        & (calc_dr(a['leading_eta'], a['leading_phi'], eta_darkquark2, phi_darkquark2)<=1.5)
        )

    # AK4 and AK8 jet info
